...
| `dama` | Dama (Türk Daması) | sıra-tabanlı, zorunlu alma | alfa-beta + uçan dama |
| `tombala` | Tombala | süreli oto-çekiliş, manuel işaretleme | kusursuz oto-işaret |
...
Native paket: **Capacitor** (Faz 5, opsiyonel).
Kullanıcı tarafı: telefonda dokunmatik test, Render'a `ANTHROPIC_API_KEY` ekleme.
Not: Dama bot-vs-bot'ta sık berabere kalır (iki eşit minimax dama oyunsonunu
çeviremez); insana karşı sorun değil. Dama'da ilerleme-yok beraberlik kuralı var
(`IDLE_LIMIT` = 50 yarım hamle) — oyunların sonlanmasını garanti eder.
